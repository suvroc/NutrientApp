...
gulp.task('partials', ['clean'], function () {
  return gulp.src([
    ...
    path.join(conf.paths.serve, '/**/*.html')
  ], { follow: true })
    .pipe($.htmlmin({
...
gulp.task('html', ['clean', 'inject', 'partials'], function () {
  ...
  return gulp.src(path.join(conf.paths.serve, '/*.html'))
    ...
    // .pipe($.sourcemaps.init())
    .pipe($.cssnano({ zindex: false })) // do not normalize z-index, we're using bootstrap's scale
    .pipe($.rev())
    // .pipe($.sourcemaps.write('maps'))
...
gulp.task('app-config-build', ['clean'],
  () => conf.copyConfiguration(conf.paths.dist)
);
...
gulp.task('build', ['clean', 'html', 'assets', 'fonts', 'components-copy', 'other', 'tslint', 'app-config-build']);
